...
impl<W:Write> BristolNXAOWriter<W> {
    ...
    pub fn write(&mut self) -> Result<(), BristolError> {
        let input_len = self.circuit.input_len();
        let output_len = self.circuit.output_len();
        let num_gate_without_inputs = self.circuit.num_gate() - input_len;
        let num_wire = self.circuit.num_wire();
        let first_line = format!("{} {}\n",num_gate_without_inputs,num_wire);
        ...
        for i in 0..output_len {
            ...
            match output_gate {
                NXAOBoolGate::Not(gate) => {
                    ...
                    self.write_single_gate(&gate.id, input_gate, &mut wire_of_gate)?;
                },
                NXAOBoolGate::Xor(gate) => {
                    let l_gate = self.circuit.get_gate(&gate.left_id)?.clone();
                    self.write_single_gate(&gate.left_id, l_gate, &mut wire_of_gate)?;
                    let r_gate = self.circuit.get_gate(&gate.right_id)?.clone();
                    self.write_single_gate(&gate.right_id, r_gate, &mut wire_of_gate)?;
                },
                ...
                _ => {
                    return  Err(BristolError::InvalidOutputGate(output_wire_id));
                }
            }
        }
        ...
        for i in 0..output_len {
            ...
            self.write_single_gate(&gate_id, gate.clone(), &mut wire_of_gate)?;
        }
        //self.writer.flush()?;
        Ok(())
    }
    ...
    fn write_single_gate(
        ...
        match gate {
            ...
            NXAOBoolGate::Not(gate) => {
                ...
                self.update_wire(gate_id, 1, Some(&[input_wire]), &gate.to_str(), wire_of_gate)
            }
            NXAOBoolGate::Xor(gate) => {
                ...
                self.update_wire(gate_id, 2, Some(&[l_wire, r_wire]), &gate.to_str(), wire_of_gate)
            }
            NXAOBoolGate::And(gate) => {
                if wire_of_gate.get(&gate.left_id).is_none() {
                    let input_gate = self.circuit.get_gate(&gate.left_id)?.clone();
